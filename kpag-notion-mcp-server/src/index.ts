...
interface PricingRule {
  brand: string;
  basePrice: number;
  albumPrices: { [album: string]: number };
  locationSurcharge: { [location: string]: number };
  specialRules: { [venue: string]: number };
}
...
class KPAGPricingEngine {
  ...
  calculatePrice(brand: string, album: string, venue: string): number {
    const rule = this.pricingRules.find(r => r.brand === brand);
    if (!rule) throw new Error(`알 수 없는 브랜드: ${brand}`);

    // 특수 예식장 규칙 확인
    const specialPrice = Object.entries(rule.specialRules).find(([key, _]) => 
      venue.includes(key)
    );
    if (specialPrice) {
...
class KPAGMessageParser {
  parseManagerMessage(message: string): ShootingData {
    const lines = message.trim().split('\n').map(line => line.trim());
    
    const data: Partial<ShootingData> = {
      숙지사항: []
    };
    ...
    // 예식장 추출
    const venueMatch = lines.find(line => 
    ...
    const productLine = lines.find(line => 
      line.includes('세븐스') || line.includes('세컨') || line.includes('그라피')
    );
    if (productLine) {
    ...
    if (specialNotes.includes('선촬영')) data.숙지사항!.push('선촬영');
    if (specialNotes.includes('폐백') && !specialNotes.includes('폐백X') && !specialNotes.includes('폐백 X')) {
      data.숙지사항!.push('폐백');
    }
    if (specialNotes.includes('포토부스')) data.숙지사항!.push('포토부스');
    if (specialNotes.includes('플래시컷')) data.숙지사항!.push('플래시컷');
    if (specialNotes.includes('드레스실') || specialNotes.includes('드래스룸')) {
      data.숙지사항!.push('드래스룸');
    }
    if (specialNotes.includes('홀스냅')) data.숙지사항!.push('홀스냅');

    // 전달사항 생성
    const additionalInfo = [];
    if (message.includes('지하')) additionalInfo.push('지하1층');
    if (message.includes('미니북')) additionalInfo.push('미니북 2권');
    if (message.includes('폐백X') || message.includes('폐백 X')) additionalInfo.push('폐백X');
    data.전달사항 = additionalInfo.join(', ');

    return data as ShootingData;
  }
}
    ...
    switch (name) {
      case 'parse_manager_message': {
        ...
        if (parsedData.브랜드 && parsedData.앨범 && parsedData.예식장) {
          parsedData.예상단가 = pricingEngine.calculatePrice(
            parsedData.브랜드, 
            parsedData.앨범, 
            parsedData.예식장
          );
      ...
      case 'create_notion_shooting_data': {
        ...
        const shootingData = args.shootingData as ShootingData;
      ...
      case 'validate_shooting_data': {
        ...
        const shootingData = args.shootingData as any;
        const errors: string[] = [];
        
        if (!shootingData || typeof shootingData !== 'object') {
          errors.push('유효하지 않은 촬영 데이터 형식');
        } else {
          if (!shootingData.신랑신부) errors.push('신랑신부 정보 누락');
          if (!shootingData.일시) errors.push('일시 정보 누락');
          if (!shootingData.예식장) errors.push('예식장 정보 누락');
          if (!shootingData.브랜드) errors.push('브랜드 정보 누락');
          if (!shootingData.앨범) errors.push('앨범 정보 누락');
          if (!shootingData.예상단가 || shootingData.예상단가 <= 0) errors.push('예상단가 정보 누락/오류');
        }
